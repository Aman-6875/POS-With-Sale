...
class Controller extends BaseController
{
    ...
    public function index()
    {
        $date = date('Y-m-d');
        $month = date('Y-m');
        $total_sell = \App\SellItem::where('created_at', 'like', '%' . $date . '%')->count();
        $grand_total_price = \App\Sell::where('created_at', 'like', '%' . $date . '%')->sum('grand_total_price');
        $total_expense = \App\Expense::where('created_at', 'like', '%' . $date . '%')->sum('expense_amount');
        $sells = Sell::
        join('customers', 'customers.customer_id', '=', 'sells.customer_id')
            ...
            ->limit(15)
            ->get();
        $this_month_total_sell = \App\Sell::where('created_at', 'like', '%' . $month . '%')->sum('grand_total_price');
        return view('test')
